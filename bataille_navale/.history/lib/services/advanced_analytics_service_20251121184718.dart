...
class AdvancedAnalyticsService {
  ...
  /// Génère un dataset de N parties simulées (ULTRA-OPTIMISÉ)
  /// - Batch size: 10000 (au lieu de 1000)
  /// - Zero allocations inutiles
  /// - Future.wait pour parallelization complète
  Future<List<GameStatistics>> generateSimulatedDataset(
    GameService gameService,
    int numberOfGames,
    AIStrategy strategy,
  ) async {
    print('🚀 ULTRA-SIMULATION: $numberOfGames parties');
    final startTime = DateTime.now();

    // Batch énorme pour moins de context switching
    ...
    for (int batchStart = 0; batchStart < numberOfGames; batchStart += batchSize) {
      ...
      // Générer le batch en parallèle (sans await intermédiaire)
      ...
      // Attendre TOUT le batch d'un coup (parallelization maximale)
      ...
      print('  ✅ ${dataset.length}/$numberOfGames | $perSecond/sec | ETA: ${eta}s');
    }
