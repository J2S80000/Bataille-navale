...
class GameService {
  ...
  static final List<GameStatistics> _simulationHistory = [];
  
  GameService();

  // ============ SIMULATION HISTORY ============

  /// Ajoute des statistiques de simulation à l'historique
  void addSimulationStats(GameStatistics stats) {
    _simulationHistory.add(stats);
    print('📊 Stat sauvegardée localement (total: ${_simulationHistory.length})');
  }

  /// Récupère l'historique des simulations
  List<GameStatistics> getSimulationHistory() => List.from(_simulationHistory);

  /// Vide l'historique des simulations
  void clearSimulationHistory() {
    _simulationHistory.clear();
    print('🗑 Historique des simulations vidé');
  }
  ...
  bool canPlaceShip(
    ...
    for (int i = 0; i < shipSize; i++) {
      ...
      // Vérifier les cellules adjacentes (pas idéal pour le jeu, mais plus réaliste)
      for (int dr = -1; dr <= 1; dr++) {
        for (int dc = -1; dc <= 1; dc++) {
          if (board.hasShip(row + dr, col + dc)) return false;
        }
      }
    }
  ...
  (MoveResult result, Game updatedGame) processMove(
    ...
    if (game.currentPlayerBoard.hasShip(row, col)) {
      ...
      if (ship != null) {
        ...
        updatedBoard = currentBoard.updateCell(row, col, CellState.hit);
    ...
    Game updatedGame = game.copyWith(
      board1: game.isPlayer1Turn ? game.board1 : updatedBoard,
      board2: game.isPlayer2Turn ? game.board2 : updatedBoard,
