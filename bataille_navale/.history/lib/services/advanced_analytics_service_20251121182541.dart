...
class AdvancedAnalyticsService {
  ...
  Future<List<GameStatistics>> generateSimulatedDataset(
    ...
    print('🚀 Simulation rapide: $numberOfGames parties...');
    final startTime = DateTime.now();

    // Diviser en batches de 100 pour paralléliser
    const batchSize = 100;
    ...
    for (int batchStart = 0; batchStart < numberOfGames; batchStart += batchSize) {
      ...
      print('  📊 Batch ${(batchStart ~/ batchSize) + 1}: $batchStart-$batchEnd (${batchCount} parties)');

      // Lancer toutes les simulations du batch en parallèle
      final futures = List.generate(batchCount, (index) async {
        try {
          ...
          print('  ⚠️ Erreur jeu ${batchStart + index}: $e');
          return null;
        }
      });
      ...
      print('  ✅ Batch terminé - ${dataset.length}/${numberOfGames} parties (${elapsed.inSeconds}s)');
    }

    final totalTime = DateTime.now().difference(startTime);
    print('✅ Simulation terminée: ${dataset.length} parties en ${totalTime.inSeconds}s');
