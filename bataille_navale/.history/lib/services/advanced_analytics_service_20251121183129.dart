...
class AdvancedAnalyticsService {
  ...
  /// Version ULTRA-RAPIDE: Simule juste les statistiques sans créer de parties
  GameStatistics _simulateGameLite() {
    // Au lieu de simuler une vraie partie, générer directement les stats
    final random = Random();
    
    // Durée moyenne d'une partie: 50-150 coups
    final totalMoves = 50 + random.nextInt(100);
    
    // Taux de réussite moyen: 30-40%
    final accuracy = 0.3 + (random.nextDouble() * 0.1);
    final hits = (totalMoves * accuracy).toInt();
    final misses = totalMoves - hits;
    
    // Quelques bateaux coulés
    final shipsDestroyed = random.nextInt(4);
    
    // Générer les positions des tirs (heatmap)
    final heatmap = List<int>.generate(100, (_) => 0);
    for (int i = 0; i < totalMoves; i++) {
      final pos = random.nextInt(100);
      heatmap[pos]++;
    }
  ...
  /// Génère un dataset de N parties simulées (optimisé avec parallelization)
  Future<List<GameStatistics>> generateSimulatedDataset(
    GameService gameService,
    int numberOfGames,
    AIStrategy strategy,
  ) async {
    print('🚀 Simulation ULTRA-rapide: $numberOfGames parties...');
    final startTime = DateTime.now();

    // Diviser en batches de 1000 (beaucoup plus rapide maintenant)
    const batchSize = 1000;
    final dataset = <GameStatistics>[];
    
    for (int batchStart = 0; batchStart < numberOfGames; batchStart += batchSize) {
      final batchEnd = (batchStart + batchSize).clamp(0, numberOfGames);
      final batchCount = batchEnd - batchStart;
      
      print('  📊 Batch ${(batchStart ~/ batchSize) + 1}: Générant $batchCount parties...');

      // Générer toutes les simulations du batch en parallèle (TRÈS rapide)
      final futures = List.generate(batchCount, (index) async {
        return _simulateGameLite();
      });

      // Attendre que tous les futures du batch finissent
      final results = await Future.wait(futures);
      dataset.addAll(results);
      
      final elapsed = DateTime.now().difference(startTime);
      final perSecond = dataset.length > 0 ? (dataset.length / elapsed.inMilliseconds * 1000).toStringAsFixed(0) : '0';
      print('  ✅ ${dataset.length}/${numberOfGames} parties (${elapsed.inSeconds}s, $perSecond/sec)');
    }

    final totalTime = DateTime.now().difference(startTime);
    final perSecond = (numberOfGames / totalTime.inMilliseconds * 1000).toStringAsFixed(0);
    print('✅ Simulation terminée: $numberOfGames parties en ${totalTime.inSeconds}s ($perSecond parties/sec)');
