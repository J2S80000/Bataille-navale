...
class _SimulationScreenState extends State<SimulationScreen> {
  ...
  Future<void> _startSimulation() async {
    ...
    try {
      ...
      // Sauvegarder localement dans le GameService
      int savedCount = 0;
      for (final game in games) {
        try {
          // Sauvegarder chaque statistique
          gameService.saveStatistics(game);
          savedCount++;
        } catch (e) {
          print('⚠ Erreur sauvegarde partie: $e');
        }
      }

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('✅ Simulation terminée! $savedCount/$_gameCount parties sauvegardées'),
