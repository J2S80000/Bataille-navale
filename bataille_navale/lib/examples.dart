...
Future<void> exampleAnalytics() async {
  ...
  if (playerStats.isNotEmpty) {
    ...
    final aggregateStats =
        await analytics.buildPlayerStatistics('alice_123', playerStats.cast<GameStatistics>());
...
''');
    ...
    for (final (row, col, count) in hotspots) {
      final pos = '${String.fromCharCode(65 + col.toInt())}${row + 1}';
      print('  - $pos: $count fois');
    }

    // Analyser les patterns
    final patterns = analytics.detectMovePatterns(playerStats.cast<GameStatistics>());
    print('\n🎯 Patterns détectés:');
    for (final entry in patterns.entries) {
      print('  - ${entry.key}: ${entry.value.length} coups');
    }

    // Calculer la prédictibilité
    final predictability = analytics.calculatePredictability(playerStats.cast<GameStatistics>());
    print(
...
Future<void> exampleAITraining() async {
  ...
  for (int gen = 0; gen < ga.generations; gen++) {
    ga.evaluateFitness(aiStats.cast<GameStatistics>());
...
Future<void> exampleAIMove() async {
  ...
  // Entraîner l'IA
  final ga = GeneticAlgorithm();
  ga.train(aiStats.cast<GameStatistics>());
  final strategy = ga.getBestStrategy();

  // Créer un prédicteur
  final predictor = MovePredictor(
    strategy: strategy,
    trainingData: aiStats.cast<GameStatistics>(),
  );
...
Future<void> exampleLeaderboard() async {
  ...
  for (int i = 0; i < topPlayers.length; i++) {
    ...
    final gamesPlayed = (player['wins'] ?? 0) + (player['losses'] ?? 0);
    final winRate = gamesPlayed > 0
        ? ((( player['wins'] ?? 0) / gamesPlayed) * 100).toStringAsFixed(1)
        : '0.0';
    print(
