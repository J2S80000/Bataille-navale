...
class _GameScreenState extends State<GameScreen> {
  ...
  Future<void> _loadAIModel() async {
    ...
    print('[AI] Joueur1 ID: ${currentGame.player1.id}');
    ...
    try {
      // Ajouter un délai pour s'assurer que MongoDB a eu le temps de répondre
      await Future.delayed(Duration(milliseconds: 200));
      
      // Charger les modèles IA
      final mongoService = MongoDBService();
      print('[AI] Début du chargement des modèles depuis MongoDB...');
      ...
      print('[AI] Modèles chargés: ${models.keys.toList()}');
      for (final entry in models.entries) {
        print('[AI]   - ${entry.key}: ${entry.value.trainingIterations} entraînements');
      }

      // Sélectionner le modèle correspondant à la difficulté
      if (models.containsKey(aiDifficulty)) {
        final model = models[aiDifficulty]!;
        print('[AI] ✓ Modèle trouvé: $aiDifficulty');
        print('[AI] Entraînements: ${model.trainingIterations}');
        print('[AI] Learning rate: ${model.instanceLearningRate}');
        ...
        if (mounted) {
          ...
          print('[AI] ✓ Modèle appliqué avec succès!');
        }
      } else {
        print('[ERREUR] Modèle $aiDifficulty NOT FOUND dans: ${models.keys.toList()}');
        if (mounted) {
      ...
      print('[ERREUR] Stack trace: $stackTrace');
      if (mounted) {
        setState(() => isLoadingModel = false);
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: GameScreenAppBar(title: appBarTitle),
