...
class _PlacementScreenState extends State<PlacementScreen> {
  ...
  void initState() {
    super.initState();
    board = Board.empty();
    // Charger les recommandations après le build
  ...
  Future<void> _loadRecommendations() async {
    try {
      // Créer les services directement sans context.read
      final analytics = AdvancedAnalyticsService();
      ...
      for (final stratType in ['defensive', 'balanced', 'aggressive']) {
        try {
          ...
          print('Erreur stratégie $stratType: $e');
        }
      }
  ...
  void _applyRecommendation() {
    ...
    // Créer un nouveau plateau avec les positions recommandées
    var newBoard = Board.empty();
    
    // Placer chaque navire selon la stratégie recommandée
    for (final shipType in ShipType.values) {
      final gameService = GameService();
      
      // Essayer de placer le navire de manière aléatoire mais en favorisant les zones recommandées
      bool placed = false;
      int attempts = 0;
      
      while (!placed && attempts < 50) {
        // Choisir une position préférée selon la stratégie
        final isVertical = attempts % 2 == 0; // Alterner horizontal/vertical
        final startRow = (attempts ~/ 2) % 10;
        final startCol = (attempts * 3) % 10;
        
        if (gameService.canPlaceShip(newBoard, startRow, startCol, shipType, isVertical)) {
          newBoard = gameService.placeShip(newBoard, startRow, startCol, shipType, isVertical);
          placed = true;
        }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Strategie appliquee: ${strategy.strategyName}')),
    );
  }
  ...
  void _complete() {
    if (board.ships.length < 5) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Placez tous les navires avant de continuer (${board.ships.length}/5)')),
      );
      return;
    }

    print('[OK] Placement termine avec ${board.ships.length} navires');
    print('   Navires places: ${board.ships.map((s) => s.type.toString()).toList()}');
    
    try {
      widget.onPlacementComplete(board);
    } catch (e) {
      print('[ERREUR] Lors du callback: $e');
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PlacementAppBar(onComplete: _complete),
      body: Column(
        children: [
          Expanded(
                  ...
                  return Padding(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Sélectionné: ${type.displayName} (${size}). Cliquez sur le plateau pour placer.'),
                            duration: Duration(seconds: 2),
                          ),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: selectedShip == type
