...
class NeuralNetworkAI {
  ...
  /// Charge les poids du réseau depuis JSON
  static NeuralNetworkAI fromJson(Map<String, dynamic> json) {
    ...
    model.inputHiddenWeights = (json['inputHiddenWeights'] as List)
        .map((row) => (row as List).map((x) => (x as num).toDouble()).toList())
        .toList();
    model.hiddenOutputWeights = (json['hiddenOutputWeights'] as List)
        .map((row) => (row as List).map((x) => (x as num).toDouble()).toList())
        .toList();
