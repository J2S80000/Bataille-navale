...
class NeuralNetworkAI {
  ...
  /// Compresse les poids en gardant seulement les biais (beaucoup plus petits)
  /// Les poids inputHidden/hiddenOutput sont recalculés depuis modelId (seed)
  Map<String, dynamic> toJson() {
    return {
      'modelId': modelId,
      'difficulty': difficulty,
      'trainingIterations': trainingIterations,
      'instanceLearningRate': instanceLearningRate,
      // Sauvegarder seulement les biais (64 + 100 = 164 nombres au lieu de 12964)
      'hiddenBias': hiddenBias,
      'outputBias': outputBias,
    };
  }

  /// Charge le modèle depuis JSON en recalculant les poids
  static NeuralNetworkAI fromJson(Map<String, dynamic> json) {
    final model = NeuralNetworkAI(
      modelId: json['modelId'] as String? ?? 'model_${DateTime.now().millisecondsSinceEpoch}',
      difficulty: json['difficulty'] as String? ?? 'medium',
      trainingIterations: json['trainingIterations'] as int? ?? 0,
      instanceLearningRate: (json['instanceLearningRate'] as num?)?.toDouble() ?? 0.01,
    );

    // Charger les biais sauvegardés
    if (json.containsKey('hiddenBias') && json['hiddenBias'] is List) {
      model.hiddenBias = (json['hiddenBias'] as List)
          .map((x) => (x as num).toDouble())
          .toList();
    }
    if (json.containsKey('outputBias') && json['outputBias'] is List) {
      model.outputBias = (json['outputBias'] as List)
          .map((x) => (x as num).toDouble())
          .toList();
    }
    
    // Les poids inputHidden/hiddenOutput sont recalculés dans _initializeWeights()
    // basé sur le modelId qui sert de seed
