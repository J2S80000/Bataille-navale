...
class AdvancedAnalyticsService {
  ...
  InitialPlacementStrategy recommendPlacementStrategy(
    ...
    switch (strategyType) {
      case 'aggressive':
        ...
        for (final (row, col, _) in heatmap.getAcceptablePositions(riskThreshold: 0.2)) {
          recommendedPositions.add((row, col, false));
      ...
      case 'defensive':
        ...
        for (final (row, col, _) in allPositions.take(5)) {
          recommendedPositions.add((row, col, false));
        }
        break;
      ...
      default:
        ...
        for (final (row, col, prob) in topPositions) {
  ...
  /// Génère un dataset de N parties simulées
  Future<List<GameStatistics>> generateSimulatedDataset(
    ...
    for (int i = 0; i < numberOfGames; i++) {
      try {
        final stats = await simulateGame(
          gameService,
          strategy,
          strategy,
          null,
        ...
        print('Erreur simulation jeu $i: $e');
      }
    }
