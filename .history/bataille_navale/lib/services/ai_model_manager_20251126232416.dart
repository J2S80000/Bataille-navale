...
class AIModelManager {
  ...
  static Future<Map<String, NeuralNetworkAI>> loadUserModels(
    ...
    try {
      ...
      print('[LOAD] Nombre de documents trouvés: ${response.length}');
      
      // Afficher tous les documents de type ai_model
      final aiModels = response.where((doc) => doc['type'] == 'ai_model').toList();
      print('[LOAD] Nombre de modèles IA trouvés: ${aiModels.length}');
      for (final doc in aiModels) {
      ...
      for (final difficulty in difficulties) {
        ...
        final modelData = response.firstWhere(
          (doc) => doc['type'] == 'ai_model' && doc['difficulty'] == difficulty,
        ...
        if (modelData.isNotEmpty) {
          try {
            ...
            print('[LOAD]   Données: trainingIterations=${modelData['trainingIterations']}');
  ...
  static Future<bool> saveModel(
    ...
    try {
      final modelData = {
        ...
        'type': 'ai_model',
        'modelUniqueId': '${playerId}_${model.difficulty}', // ID unique pour chaque modèle
