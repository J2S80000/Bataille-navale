...
class _GameScreenState extends State<GameScreen> {
  ...
  void _playAIMove(GameService gameService) {
    try {
      ...
      // Créer une stratégie IA par défaut (équilibrée)
      final defaultStrategy = AIStrategy(
        id: 'default-ai',
        weights: [0.2, 0.3, 0.2, 0.2, 0.1],  // Poids équilibrés
        fitness: 0.0,
      );

      // Créer un prédicteur IA avec données d'entraînement vides (jeu simple)
      ...
      final (row, col) = predictor.predictNextMove(currentGame.board1, aiMoves);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Bataille Navale - ${currentGame.currentPlayer.name} vs ${currentGame.opponent.name}'),
        elevation: 0,
