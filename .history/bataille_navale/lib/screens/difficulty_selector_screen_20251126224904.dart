...
class _DifficultySelectorScreenState extends State<DifficultySelectorScreen> {
  ...
  Future<void> _loadModelsStats() async {
    try {
      ...
      if (mounted) {
        setState(() {
          modelsStats = stats.fold<Map<String, dynamic>>(
            {},
            (acc, model) {
              acc[model.difficulty] = {
  ...
  String _getStrengthBar(int trainingIterations) {
    ...
    if (trainingIterations < 5) return '▓░░░░ (Débutant)';
    if (trainingIterations < 10) return '▓▓░░░ (Apprenti)';
    if (trainingIterations < 20) return '▓▓▓░░ (Confirmé)';
    if (trainingIterations < 50) return '▓▓▓▓░ (Expert)';
    return '▓▓▓▓▓ (Maître)';
  }
