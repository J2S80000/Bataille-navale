...
class _DifficultySelectorScreenState extends State<DifficultySelectorScreen> {
  ...
  String _getStrengthBar(int trainingIterations) {
    // Afficher la force croissante avec les entraînements
    // Les seuils sont ajustés car chaque difficulté reçoit un nombre d'epochs différent par entraînement:
    // Easy: +5 epochs | Medium: +10 epochs | Hard: +15 epochs | Expert: +20 epochs
    
    if (trainingIterations == 0) return '░░░░░ (Non entraîné)';
    if (trainingIterations < 10) return '▓░░░░ (Débutant)';        // 1-2 entraînements
    if (trainingIterations < 30) return '▓▓░░░ (Apprenti)';        // 3-6 entraînements  
    if (trainingIterations < 60) return '▓▓▓░░ (Confirmé)';        // 7-12 entraînements
    if (trainingIterations < 120) return '▓▓▓▓░ (Expert)';         // 13-24 entraînements
    return '▓▓▓▓▓ (Maître)';                                       // 25+ entraînements
  }
