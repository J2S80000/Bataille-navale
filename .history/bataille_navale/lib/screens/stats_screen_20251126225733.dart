...
class _StatsScreenState extends State<StatsScreen> {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // Entraîner tous les modèles IA
              await _trainAllAIModels(
                placementPatterns: placementPatterns,
                attackPatterns: attackPatterns,
              );
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('✓ Les IA ont été entraînées avec votre profil'),
  ...
  Future<void> _trainAllAIModels({
    ...
    try {
      ...
      // Entraîner chaque modèle avec les patterns du joueur
      for (final difficulty in ['easy', 'medium', 'hard', 'expert']) {
        if (models.containsKey(difficulty)) {
          var model = models[difficulty]!;
          
          print('[TRAIN] Entraînement du modèle $difficulty...');
          
          // Entraîner le modèle avec les statistiques du joueur
          model = await AIModelManager.trainModel(
            model: model,
          ...
          // Sauvegarder le modèle entraîné en base de données
          ...
          print('[TRAIN] Modèle $difficulty entraîné avec succès (${model.trainingIterations} entraînements)');
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erreur lors de l\'entraînement des IA: $e'),
