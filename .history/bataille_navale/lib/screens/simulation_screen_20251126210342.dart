  ...
  Future<void> _startSimulation() async {
    ...
    try {
      ...
      for (int i = 0; i < _gameCount; i++) {
        try {
          // Créer une partie simulée
          final board1 = gameService.generateRandomShipPlacement();
          final board2 = gameService.generateRandomShipPlacement();
          
          // Créer des stratégies fictives pour la simulation
          final strategy1 = AIStrategy(weights: [0.5, 0.5, 0.5, 0.5, 0.5]);
          final strategy2 = AIStrategy(weights: [0.6, 0.4, 0.5, 0.5, 0.5]);
          
          // Simuler le jeu
          final stats = await analytics.simulateGame(
            gameService,
            strategy1,
            strategy2,
            board1,
      ...
      if (_results.isNotEmpty) {
        final aggregate = await AnalyticsService().buildPlayerStatistics(
          widget.playerId,
          _results,
        );
        
        setState(() {
          _aggregateStats = aggregate;
          _isSimulating = false;
        });
      } else {
        setState(() {
